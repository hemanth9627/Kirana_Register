...
public class Transaction_Service {
    ...
    ********************
    ...
    if (transactionRegister.getCredit_amount()==null || ((String) transactionRegister.getCredit_amount()).isEmpty())
        transactionRegister.setCredit_amount("0.00");
    BigDecimal creditAmount = new BigDecimal((char[]) transactionRegister.getCredit_amount()).multiply(usdRate);
    transactionRegister.setCredit_amount(creditAmount.toString());

    if(transactionRegister.getDebit_amount()==null || ((String) transactionRegister.getDebit_amount()).isEmpty())
        transactionRegister.setDebit_amount("0.00");

    BigDecimal debitAmount = new BigDecimal((char[]) transactionRegister.getDebit_amount()).multiply(usdRate);
     ...
     * */
    @Transactional(readOnly = true)
    public Map<Object, List<Transaction_Register>> getDailyReports() {
